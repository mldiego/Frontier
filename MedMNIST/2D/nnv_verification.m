...
benchmarks = dir(pwd);
n_bench = height(benchmarks);

% Analyze all benchmarks
for i=3:height(benchmarks)
    if benchmarks(i).isdir
        % Get info on NN and specifications
        networks = dir(fullfile(benchmarks(i).name, 'onnx', '*.onnx'));
        specs = dir(fullfile(benchmarks(i).name, 'vnnlib', '*.vnnlib'));

        % Begin for loop to analyzing evry onnx model
        for o = 1:height(networks)
            ...
            onnxPath = fullfile(networks(o).folder, networks(o).name);
            % load NN from onnx
            net = importNetworkFromONNX(onnxPath, "InputDataFormats", "BCSS", "OutputDataFormats","BC");
            % transform to NNV
            ...
            for p = 1:height(specs)
                
                % Get path to vnnlib specification
                specPath = fullfile(specs(p).folder, specs(p).name);

                % Create path to utput file (to save results)
                outputfile = fullfile("../results", specs(p).name);
